...
import (
	...
	openshiftdefaults "github.com/openshift/openshift-apiserver/pkg/api/install"
...
func init() {
	...
	openshiftdefaults.InstallInternalKube(myscheme)
	openshiftdefaults.InstallInternalOpenShift(myscheme)
	decoder = serializer.NewCodecFactory(myscheme).UniversalDeserializer()
}

// NeatDefaults gets a json document representing a OpenShift resource, and removes all fields with default values.
// default values is determined by invoking the "defaulting" code from OpenShift apiserver
